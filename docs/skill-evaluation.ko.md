...
`validate_evals.py`에 등록한 라우팅 정의만 허용하며, 현재는 `routing.json`입니다.
실제 라우팅 형식의 정의를 추가하려면 `scripts/validate_evals.py`의 `ROUTING_FILES`에
파일명을 추가합니다. 행동 평가 스위트는 `evals/suites/`에 둡니다.
라우팅 검증기가 등록된 모든 파일을 검사하고, 그 외 JSON이 최상위에 있으면
이동할 위치를 안내하며 실패합니다.
...
스위트·실행 기록·판정 파일은 형식 버전을 따로 관리하며, 현재 각각 1·3·1입니다.
실행 기록은 현재 형식만 허용합니다. 개발 중 만들어진 버전 1·2 기록은 지원하지
않는 버전이라는 오류로 거부하며, 원본을 고치거나 자동 변환하지 않습니다.
이전 파일은 과거의 근거로 보존하고, 현재 도구에서 판정·비교할 실험은 양쪽
조건을 새로 실행합니다. 형식 검사를 통과하려고 기록의 버전 번호만 바꾸지 않습니다.
...
같은 스킬의 독립적인 실행은 `--variability`를 명시해 비교합니다.
**SAME-SKILL VARIABILITY** 모드로
...
하네스·실행 형식·모델·환경·선택한 평가 사례와 기준·runner 및 명시적 실행 파일·
supervisor의 Python 인터프리터·split·
...
현재 실행은 선택한 사례를 ID 순으로 정렬해 실제 호출 순서와 비교 해시를
...
`work/eval/current.json`과 각각의 판정 파일을 연결하고 `--variability`를 지정해
비교합니다. 이 옵션이 없으면 같은 내용의
스킬 비교를 거부하며, 반대로 스킬 내용이 다르면 이 옵션을 거부합니다. 기본
비교는 스킬 변경에 사용합니다. 디렉터리 이름만 바꾸어도 파일 내용이 같으면
같은 스킬로 봅니다.
...
스킬 파일의 변경 여부가 요청한 비교 모드와 일치해야 합니다.
...
runner의 실행 파일·명시적 파일 인자는 `runner_identity`에, supervisor를 시작하는
Python 인터프리터는 `supervisor_identity`에 명령과 파일 해시를 기록합니다.
둘 다 실험 시작 전과 종료 후에 한 번씩 해시를 검사합니다. 각 시도 전에는
장치·inode·크기·수정 시간·변경 시간·모드의 변화를 확인해 일반적인 중간 변경을
막습니다. 매번 큰 실행 파일을 다시 해시하지는 않으며,
악의적인 위변조나 adapter가 불러오는 모든 의존성·운영체제를 검증하는 기능도
아닙니다. 통제할 수 있는 의존성의 버전을 고정하고 환경 설명에 기록합니다.

시간 제한은 자식 프로세스가 상속한 출력 파이프에도 적용합니다. 같은 프로세스
그룹에 supervisor를 살려 두고 전용 파이프로 실제 runner의 종료 상태를 받습니다.
별도의 생존 확인 파이프는 harness만 쓰기 끝을 소유합니다. supervisor는 runner의
실행 중과 종료 후 모두 이 파이프를 확인하므로, harness가 SIGTERM·SIGKILL로
종료돼도 EOF를 받으면 자기 프로세스 그룹에 종료 신호를 보냅니다.
성공·오류·시간 초과·중단 경로에서는 harness가 그룹에 신호를 보낸 뒤 supervisor를
회수하며, 신호 전송이 실패해도 생존 확인 파이프를 닫습니다. 표준 입출력만 닫고
남아 있는 후손도 같은 정리 대상에 포함됩니다. 정리 과정의 대기와 재시도는 총
1초로 제한하며, 넘으면 프로세스가 아직 실행 중일 수 있다는 진단을 남기고 반환합니다.
...
supervisor에서 유효한 runner 종료 상태를 받지 못했거나 정리가 실패하면 기존
상태와 관계없이 `error`에 진단을 추가합니다. 첫 상태 쓰기를 시도하기 전에는
`Supervisor failure:` 또는 `Supervisor waiter failure:`와 traceback으로 내부
예외를 보고할 수도 있으며, 이때 `exit_code`는 `null`입니다. 이는 상태 메시지가
유실되거나 잘못된 경우와 달리 내부 실패를 보고한 경우입니다. 쓰기가 시작된
뒤에는 두 번째 JSON을 덧붙이지 않습니다. 온전히 받은 첫 메시지의 runner 상태는
보존하고, 일부만 받은 메시지는 유효한 결과로 인정하지 않습니다. 따라서 늦게
발생한 예외의 traceback은 남지 않을 수 있지만, 정리 실패는 별도로 보고합니다.
기록된 실패 진단은 성공 상태를 오류로 바꾸고, 이미 시간 초과였다면
`timeout` 상태를 유지하면서 추가 진단을 남깁니다. `setsid`·`setpgid`로 그룹을
벗어나거나 권한을 바꾼 프로세스, 커널에서 중단할 수 없는 상태까지 정리를
보장하지는 않습니다. 보안 샌드박스가 필요하면 runner의 격리 환경에서 통제해야 합니다.
